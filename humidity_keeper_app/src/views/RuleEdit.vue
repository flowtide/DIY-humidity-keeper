<template>
  <div class="rule-view-container">
    ...
    <b-form @submit="onRuleSave" :disabled="form.isDisabled">
      <div class="m-3">
        <h4 v-if="isEditMode">
          <b-icon icon="gear-fill" aria-hidden="true"></b-icon> 규칙 수정
        </h4>
        <h4 v-if="!isEditMode">
          <b-icon icon="gear-fill" aria-hidden="true"></b-icon> 규칙 추가
        </h4>
      </div>
      ...
      <b-card>
        <div>
          ...
          <b-input-group prepend="습도하한값" class="mb-2 mr-sm-2 mb-sm-2">
            <b-form-input
              v-model="form.rule.humidityThreshold"
              :disabled="form.isDisabled"
              type="number"
              required
            >
            </b-form-input>
            &nbsp;&nbsp;<i>습도하한값 이하시 경보 발생: 예) 50±10%</i>&nbsp;
          </b-input-group>

          <b-input-group prepend="습도상한값" class="mb-2 mr-sm-2 mb-sm-2">
            <b-form-input
              v-model="form.rule.humidityHighThreshold"
              :disabled="form.isDisabled"
              type="number"
              required
            >
        ...
        <hr />
        <div>
          <b-button v-on:click="onRuleSave" variant="primary"> 저장 </b-button>
          &nbsp;
          <b-button
            v-on:click="onRuleDelete"
            variant="danger"
            v-if="isEditMode"
          >
            삭제
...
export default {
  ...
  methods: {
    async onRuleSave(evt) {
      evt.preventDefault();
      if (
        this.form.rule.humidityHighThreshold <=
        this.form.rule.humidityThreshold + 5
      ) {
        alert("습도 상한값은 하한값보다 5% 이상 커야됩니다.");
        return;
      }
      this.form.isDisabled = true;
      try {
        if (this.isEditMode) {
          let res = await this.axios.put("/api/v1/rules/update", {
            rule: this.form.rule,
          });
          if (res.data.error) {
          ...
          let res = await this.axios.post("/api/v1/rules/add", {
            rule: this.form.rule,
          });
          if (res.data.error) {
            alert("Update error: " + res.data.error.message);
    ...
    async onRuleDelete(evt) {
      ...
      try {
        let res = await this.axios.delete(
          "/api/v1/rules/delete/" + this.rule.id
        );
        if (res.data.error) {
  ...
  async mounted() {
    ...
    if (this.$route.params.id == "new") {
      ...
      let res = await this.axios.get(
        "/api/v1/rules/one?id=" + this.$route.params.id
      );
